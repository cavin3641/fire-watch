...
※ 기사 제목·링크는 구글 뉴스 것만 기록합니다.
  네이버 검색 결과는 약관상 저장하지 않고, 찾은 건수만 남깁니다.
"""
import csv
import os
...
def _row(fire):
    ...
    return {
        ...
        # 네이버는 약관상 내용은 저장하지 않고 찾은 건수만 남깁니다
        "네이버뉴스": str(fire.get("naver_count", {}).get("뉴스", 0)),
        "네이버블로그": str(fire.get("naver_count", {}).get("블로그", 0)),
        "네이버카페": str(fire.get("naver_count", {}).get("카페", 0)),
    }
