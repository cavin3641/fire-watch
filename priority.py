# -*- coding: utf-8 -*-
"""우선순위 점수. 뉴스에 크게 난 현장은 경쟁자가 몰리므로 감점합니다."""
# 대형화재(시장,공장)가 복구 규모가 가장 큽니다
# 우리 회사가 잘하는 일 = 아파트 · 주택 · 상가
# 아파트는 화재보험이 반드시 있어서 계약으로 이어질 확률이 가장 높습니다.
# 공장은 규모는 크지만 초보 영업사원이 감당하기 어려워 낮게 둡니다.
BUILDING_SCORE = {"고층건물": 45, "아파트": 45, "일반화재": 35, "대형화재": 25}

# 시설 종류가 없는 '기타화재'는 건물인지 불확실하므로 낮게
OTHER_FIRE_SCORE = 8
# 출동 차수는 시도마다 표기 관행이 다릅니다.
# 서울은 거의 모두 '2차출동', 경기·인천은 거의 모두 '1차출동'으로 적습니다.
# 그대로 점수에 쓰면 서울 건만 위로 올라오므로,
# '그 지역에서 흔한 표기인지'를 보고 판단합니다.
DISPATCH_SCORE = {"3차출동": 35, "2차출동": 22, "1차출동": 0}

# 지역별로 '2차출동' 표기가 얼마나 흔한지 (수집 때마다 다시 계산)
_scope_common = {}


def learn_scope_pattern(fires):
    """지역별 출동차수 표기 관행을 파악합니다.

    어떤 시도에서 2차출동이 절반을 넘으면 그건 '큰 불'이 아니라
    그냥 그 지역의 기본 표기입니다. 그럴 땐 가산점을 주지 않습니다.
    """
    _scope_common.clear()
    tally = {}
    for f in fires:
        if f.get("source") != "소방출동":
            continue
        sido = (f.get("region") or "").split()[0] if f.get("region") else ""
        if not sido:
            continue
        t = tally.setdefault(sido, [0, 0])
        t[0] += 1
        if "2차" in (f.get("scope") or "") or "3차" in (f.get("scope") or ""):
            t[1] += 1
    for sido, (total, high) in tally.items():
        # 절반 넘게 2차출동이면 = 그 지역 기본 표기 = 변별력 없음
        _scope_common[sido] = (total >= 5 and high / total > 0.5)
    return _scope_common
# 소방 진행 단계별 점수. 불이 꺼져야 방문할 수 있습니다.
STATUS_SCORE = {
    # 끝남 — 지금 가도 됨
    "상황종료": 15, "귀소완료보고": 15, "귀소보고": 12, "완전진화": 15, "구조완료": 12,
    # 거의 끝남
    "초진": 5,
    # 진행 중 — 아직 가면 안 됨
    "현장도착": 0, "현장도착보고": 0, "출동보고": -5,
    "출동지령": -8, "추가출동지령": -8, "출동중": -8,
}

# 영업 담당자가 보기 쉽게 세 가지로 묶습니다
STATUS_GROUP = {
    "상황종료": "방문가능", "귀소완료보고": "방문가능", "귀소보고": "방문가능",
    "완전진화": "방문가능", "구조완료": "방문가능",
    "초진": "곧가능",
    "현장도착": "진화중", "현장도착보고": "진화중", "출동보고": "진화중",
    "출동지령": "진화중", "추가출동지령": "진화중", "출동중": "진화중",
}


def status_label(fire):
    """지금 가도 되는지 한 마디로."""
    return STATUS_GROUP.get(fire.get("status") or "", "확인필요")
NEWS_PENALTY = -25

# ── 누가 갈 건물인가 ──────────────────────────────
# 영업사원이 모두 처음이라 아파트·주택·상가·빌딩만 맡깁니다.
# 그 밖의 건(공장·숙박·요양병원·학교, 종류 미상)은 모두 사장님이 봅니다.
PARTNER_KINDS = ["아파트", "상가·빌딩", "주택·빌라"]


def is_partner_case(fire):
    """영업사원 권역 방으로 보낼 건인지."""
    bk = fire.get("bkind")
    if bk:
        return bk in PARTNER_KINDS
    # 건물 이름은 없어도 소방청 분류가 '고층건물(3층이상,아파트)'면
    # 아파트나 빌딩입니다. '일반화재(차량,주택)'는 차량일 수도 있어 사장님께.
    return "고층건물" in (fire.get("kind") or "")

# 거리 가산점 사용 여부.
# 지역별로 담당자를 두면 사무실 거리는 의미가 없으므로 False 로 둡니다.
# (경기도는 넓어서 평균 39km, 서울은 20km라 서울만 유리해집니다)
USE_DISTANCE_BONUS = False
def score(fire):
    kind = fire.get("kind") or ""
    if "차량)" in kind:
        return 0
    pts = 5
    # 건물 이름으로 종류를 알아냈으면 그걸 우선합니다
    bkind = fire.get("bkind")
    # 건물 이름으로 종류를 알아냈을 때의 배점
    BK = {"아파트": 45,        # 보험 확실, 찾기 쉬움, 초보에게 최적
          "상가·빌딩": 38,     # 영업 손실 때문에 결정이 빠름
          "주택·빌라": 35,     # 우리 주력
          "공장·창고": 25,     # 규모는 크나 초보에겐 어려움
          "숙박": 22,
          "요양·병원": 15,     # 사장님 판단 — 뒤로
          "교육·종교": 10}     # 입찰·서류가 많아 초보에게 부적합
    if bkind in BK:
        pts += BK[bkind]
        matched = True
    else:
        matched = False

    for key, val in BUILDING_SCORE.items():
        if matched:
            break
        if key in kind:
            pts += val
            break
    # 그 지역에서 2차출동이 기본 표기면 가산점을 주지 않습니다
    sido = (fire.get("region") or "").split()[0] if fire.get("region") else ""
    if not _scope_common.get(sido, False):
        scope = fire.get("scope") or ""
        for key, val in DISPATCH_SCORE.items():
            if key in scope:
                pts += val
                break
    pts += STATUS_SCORE.get(fire.get("status") or "", 0)
    if fire.get("in_news"):
        pts += NEWS_PENALTY
    # 기타화재는 건물 여부가 불확실합니다
    if "기타" in kind:
        pts += OTHER_FIRE_SCORE

    # 최근 화재일수록 먼저 갑니다 (오래된 건은 이미 다른 업체가 다녀갔을 확률)
    pub = fire.get("published") or ""
    if pub:
        try:
            from datetime import datetime, timezone, timedelta
            KST = timezone(timedelta(hours=9))
            hours = (datetime.now(KST) - datetime.fromisoformat(pub)).total_seconds() / 3600
            if hours <= 12:
                pts += 18
            elif hours <= 24:
                pts += 12
            elif hours <= 48:
                pts += 5
            else:
                pts -= 8
        except Exception:
            pass

    dist = fire.get("distance_km") if USE_DISTANCE_BONUS else None
    if isinstance(dist, (int, float)):
        if dist <= 15:
            pts += 12
        elif dist <= 30:
            pts += 6
        elif dist > 60:
            pts -= 10
    return max(0, min(100, pts))
def grade(pts):
    if pts >= 78:
        return "★★★"
    if pts >= 68:
        return "★★"
    if pts >= 55:
        return "★"
    return ""
def mark_news_overlap(fires):
    """같은 시군구에 뉴스가 있으면 표시합니다 (경쟁자도 아는 건)"""
    news = [f for f in fires if f.get("source") != "소방출동"]
    for d in [f for f in fires if f.get("source") == "소방출동"]:
        d_area = " ".join((d.get("region") or "").split()[:2])
        for n in news:
            if d_area and d_area == " ".join((n.get("region") or "").split()[:2]):
                d["in_news"] = True
                d["news_url"] = n.get("url")
                break
    return fires
