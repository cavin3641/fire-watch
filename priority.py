...
# ── 누가 갈 건물인가 ──────────────────────────────
# 영업 파트너는 아파트·상가·주택만 맡습니다.
# 공장·숙박·요양병원·학교는 규모나 절차가 달라 사장님이 직접 봅니다.
PARTNER_KINDS = ["아파트", "상가·빌딩", "주택·빌라"]
OWNER_KINDS = ["공장·창고", "숙박", "요양·병원", "교육·종교"]


def is_partner_case(fire):
    """영업 파트너에게 보낼 건인지."""
    bk = fire.get("bkind")
    if bk in OWNER_KINDS:
        return False          # 사장님이 직접 볼 건
    if bk in PARTNER_KINDS:
        return True
    # 건물 이름을 못 찾은 건(분류 미상)은 파트너에게 보냅니다.
    # 아파트·주택일 확률이 높습니다.
    return True
